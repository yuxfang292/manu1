...
async function generatePDFFromMarkdown(markdownContent: string, reportData: any): Promise<Buffer> {
  // Create a well-formatted text document since PDF library has compatibility issues
  const textContent = `
...
Report generated by CUBOT
${new Date().toLocaleString()}

This report contains research findings based on regulatory documents
and compliance guidelines from authoritative sources.
`;
...
User question: ${message}`,
  ...
  app.post('/api/reports/download', async (req, res) => {
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="regulatory-report-${reportData.createdAt.toISOString().split('T')[0]}.txt"`);
      res.send(pdfBuffer);
    } catch (error) {
